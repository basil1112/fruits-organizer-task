...
const DimensionPopUp: React.FC<DimensionPopUpProps> = ({ isRectDialogVisible, userWidth, userHeight, setUserRectWidth, setUserRectHeight, validateDimension, setRectDialogVisible }) => {
    ...
    return (
        <Dialog header="Box Dimensions" visible={isRectDialogVisible} style={{ width: '50vw' }} onHide={() => setRectDialogVisible(false)}>
            <div className='container'>
                <div className='row line_spacing'>
                    <table>
                        <tbody>
                            <tr>
                                <td><input type='number' className='form-control' value={userWidth} placeholder='Enter Box Width' onChange={(e) => { setUserRectWidth(e.target.value) }} /></td>
                                <td><input type='number' className='form-control' value={userHeight} placeholder='Enter Box Height' onChange={(e) => { setUserRectHeight(e.target.value) }} /></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
